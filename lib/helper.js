...
module.exports.parseOptions = function(opts){
	if(typeof opts == "undefined"){
		...
		else if(typeof opts.resolve != "undefined"){
			var resolve = opts.resolve,
				events  = ["request", "success", "fail", "cached"];

			if(!utils.isObject(resolve)){
				throw new Error("Resolve must be a map object");
			}

			// Throw an error if any of the listed events is
			// not a valid passed value
			events.forEach(function(e){
				if(utils.inKeyObject(resolve, e) && !utils.isFunction(resolve[e])){
					throw new Error(`Resolve ${e} must be a function`);
				}
			});
		}
